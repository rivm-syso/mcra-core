...
namespace MCRA.Simulation.OutputGeneration.Views {
    public class HbmDistributionBySubstanceSectionView<S, T, T2>
        : SectionView<HbmConcentrationByDescriptorSectionBase<S, T, T2>>
        where S : IHbmExposureContributorKey, new()
        where T : HbmConcentrationDistributionRecordBase<S>, new()
                ...
                if (positivesRecords.Any()) {
                    ...
                    foreach (var item in Model.HbmBoxPlotRecords) {
                        ...
                        var chartCreator = new HbmConcentrationBySubstanceBoxPlotChartCreator<S,T2>(
                            ...
                            item.Key,
                            Model.SectionId,
                            Model.HbmBoxPlotRecords[item.Key].FirstOrDefault()?.Unit ?? string.Empty,
                            Model.ShowOutliers
                        );
