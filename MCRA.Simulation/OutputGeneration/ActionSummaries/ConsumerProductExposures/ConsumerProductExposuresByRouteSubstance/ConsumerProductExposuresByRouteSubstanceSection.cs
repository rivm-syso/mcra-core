...
namespace MCRA.Simulation.OutputGeneration {
    public class ConsumerProductExposuresByRouteSubstanceSection : SummarySection {
        protected readonly double _upperWhisker = 95;
        public override bool SaveTemporaryData => true;

        protected static double[] _percentages = [5, 10, 25, 50, 75, 90, 95];
        public List<ConsumerProductExposuresByRouteSubstanceRecord> ConsumerProductExposuresByRouteSubstanceRecords { get; set; } = [];
        ...
        public void Summarize(
            ...
            var cpExposuresByRouteRecords = new List<ConsumerProductExposuresByRouteSubstanceRecord>();
            ...
            ConsumerProductExposuresByRouteSubstanceRecords = [.. cpExposuresByRouteRecords
                .OrderBy(r => r.ExposureRoute)
                .ThenBy(r => r.SubstanceName)];

            foreach (var route in routes) {
                var exposurePercentilesRecords =
                    getBoxPlotRecords(
                        route,
                        cpIndividualDayExposures,
                        substances,
                ...
                if (exposurePercentilesRecords.Count > 0) {
                    ConsumerProductExposuresBoxPlotRecords[route] = exposurePercentilesRecords;
                }
            }
        }

        private static ConsumerProductExposuresByRouteSubstanceRecord getSummaryRecord(
            double[] percentages,
            ExposureRoute route,
            ICollection<ConsumerProductIndividualExposure> cpIndividualDayExposures,
            Compound substance,
            ...
            var record = new ConsumerProductExposuresByRouteSubstanceRecord {
