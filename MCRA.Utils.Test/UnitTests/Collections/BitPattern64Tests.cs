...
namespace MCRA.Utils.Test.UnitTests {
    [TestClass]
    public class BitPattern64Tests {
        [TestMethod]
        public void BitPattern64TestSubset() {
            var bpBas = new BitPattern64("00000110101010101001101011101111000000001111");
            var bpSub = new BitPattern64("00000110101010101001101011101111000000001111");
            Assert.IsTrue(bpSub.IsSubSetOf(bpBas));

            bpBas = new BitPattern64("00000110101010101001101011101111000000001111");
            bpSub = new BitPattern64("00000110101010101001101011100000000000001111");
            Assert.IsTrue(bpSub.IsSubSetOf(bpBas));

            bpBas = new BitPattern64("00000110101010101001101011101111000000001111");
            bpSub = new BitPattern64("0");
            Assert.IsFalse(bpSub.IsSubSetOf(bpBas));

            bpBas = new BitPattern64("00000110101010101001101011101111000000001111");
            bpSub = new BitPattern64("1");
            Assert.IsFalse(bpSub.IsSubSetOf(bpBas));

            bpBas = new BitPattern64("");
            bpSub = new BitPattern64("");
            Assert.IsFalse(bpSub.IsSubSetOf(bpBas));

            bpBas = new BitPattern64("1");
            bpSub = new BitPattern64("1");
            Assert.IsTrue(bpSub.IsSubSetOf(bpBas));

            bpBas = new BitPattern64("00000110101010101001101011101111000000001111");
            bpSub = new BitPattern64("00000000000000000000000000000000000000000001");
            Assert.IsTrue(bpSub.IsSubSetOf(bpBas));

            bpBas = new BitPattern64("00000110101010101001101011101111000000001111");
            bpSub = new BitPattern64("10000000000000000000000000000000000000000000");
            Assert.IsFalse(bpSub.IsSubSetOf(bpBas));

            bpBas = new BitPattern64("0000011010101010100110101110111100000000111");
            bpSub = new BitPattern64("0000011010100000000000000000000000000000111000000000000000000000");
            Assert.IsFalse(bpSub.IsSubSetOf(bpBas));

            bpBas = new BitPattern64("0000011010101010100110101110111100000000111");
            bpSub = new BitPattern64("000001101010000000000000000000000000000011100000000000000000000000000000010000");
            Assert.IsFalse(bpSub.IsSubSetOf(bpBas));
        }

        [TestMethod]
        public void BitPattern64GroupingTest() {
            var list = new string[] {
            ...
            var groups = list.Select(s => new BitPattern64(s))
                .GroupBy(p => p).ToList();
        ...
        [TestMethod]
        public void BitPattern64Test() {
            var strIn = "";
            var bp = new BitPattern64(strIn);
