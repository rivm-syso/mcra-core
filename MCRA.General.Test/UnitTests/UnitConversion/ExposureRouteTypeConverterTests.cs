...
namespace MCRA.General.Test.UnitTests.UnitConversion {
    ...
    [TestClass]
    public class ExposureRouteTypeConverterTests {

        [TestMethod]
        public void ExposureRouteTypeConverterTests_TestCompleteness() {
            var definition = ExposureRouteTypeConverter.UnitDefinition;
            var enumValues = Enum.GetValues(typeof(ExposureRouteType)).Cast<ExposureRouteType>().Where(r => r != ExposureRouteType.Undefined);
            // Check whether there is a unit definition for each enum value.
            foreach (var value in enumValues) {
                var unitValueDefinition = definition.FromString<ExposureRouteType>(value.ToString());
            ...
            foreach (var units in definition.Units) {
                var value = Enum.Parse(typeof(ExposureRouteType), units.Id);
            }
        }
        ...
        /// </summary>
        [TestMethod]
        public void ExposureRouteTypeConverterTests_TestDisplayNames() {
            var definition = ExposureRouteTypeConverter.UnitDefinition;
            foreach (var units in definition.Units) {
                var value = (ExposureRouteType)Enum.Parse(typeof(ExposureRouteType), units.Id);
