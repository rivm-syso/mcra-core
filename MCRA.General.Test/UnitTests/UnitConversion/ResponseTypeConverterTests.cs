...
namespace MCRA.General.Test.UnitTests.UnitConversion {
    ...
    public class ResponseTypeConverterTests {
        ...
        [TestMethod]
        public void ResponseTypeConverterTests_TestCompleteness() {
            var definition = ResponseTypeConverter.UnitDefinition;
            var enumValues = Enum.GetValues(typeof(ResponseType)).Cast<ResponseType>();
            // Check whether there is a unit definition for each enum value.
            foreach (var value in enumValues) {
                var unitValueDefinition = definition.FromString<ResponseType>(value.ToString());
            ...
            foreach (var units in definition.Units) {
                var value = Enum.Parse(typeof(ResponseType), units.Id);
            }
        }
        ...
        /// </summary>
        [TestMethod]
        public void ResponseTypeConverterTests_TestDisplayNames() {
            var definition = ResponseTypeConverter.UnitDefinition;
            foreach (var units in definition.Units) {
                var value = (ResponseType)Enum.Parse(typeof(ResponseType), units.Id);
