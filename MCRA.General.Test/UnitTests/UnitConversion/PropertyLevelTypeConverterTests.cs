...
namespace MCRA.General.Test.UnitTests.UnitConversion {
    ...
    [TestClass]
    public class PropertyLevelTypeConverterTests {

        [TestMethod]
        public void PropertyLevelTypeConverterTests_TestCompleteness() {
            var definition = PropertyLevelTypeConverter.UnitDefinition;
            var enumValues = Enum.GetValues(typeof(PropertyLevelType)).Cast<PropertyLevelType>();
            // Check whether there is a unit definition for each enum value.
            foreach (var value in enumValues) {
                var unitValueDefinition = definition.FromString<PropertyLevelType>(value.ToString());
            ...
            foreach (var units in definition.Units) {
                var value = Enum.Parse(typeof(PropertyLevelType), units.Id);
            }
        }
        ...
        /// </summary>
        [TestMethod]
        public void PropertyLevelTypeConverterTests_TestDisplayNames() {
            var definition = PropertyLevelTypeConverter.UnitDefinition;
            foreach (var units in definition.Units) {
                var value = (PropertyLevelType)Enum.Parse(typeof(PropertyLevelType), units.Id);
