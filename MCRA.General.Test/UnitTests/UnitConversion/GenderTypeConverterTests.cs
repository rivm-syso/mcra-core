...
namespace MCRA.General.Test.UnitTests.UnitConversion {
    ...
    [TestClass]
    public class GenderTypeConverterTests {

        [TestMethod]
        public void GenderTypeConverterTests_TestCompleteness() {
            var definition = GenderTypeConverter.UnitDefinition;
            var enumValues = Enum.GetValues(typeof(GenderType)).Cast<GenderType>().Where(r => r != GenderType.Undefined);
            // Check whether there is a unit definition for each enum value.
            foreach (var value in enumValues) {
                var unitValueDefinition = definition.FromString<GenderType>(value.ToString());
            ...
            foreach (var units in definition.Units) {
                var value = Enum.Parse(typeof(GenderType), units.Id);
            }
        }
        ...
        /// </summary>
        [TestMethod]
        public void GenderTypeConverterTests_TestDisplayNames() {
            var definition = GenderTypeConverter.UnitDefinition;
            foreach (var units in definition.Units) {
                var value = (GenderType)Enum.Parse(typeof(GenderType), units.Id);
