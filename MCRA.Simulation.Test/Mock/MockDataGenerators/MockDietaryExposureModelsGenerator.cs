...
namespace MCRA.Simulation.Test.Mock.MockDataGenerators {
    ...
    /// </summary>
    public static class MockDietaryExposureModelsGenerator {
        ...
        /// <returns></returns>
        public static ICollection<DietaryExposureModel> CreateMockDietaryExposureModels(
            string[] names,
            double[] percentages,
            int numUncertaintyRecords,
            IRandom random
        ) {
            var result = new List<DietaryExposureModel>();
            foreach (var name in names) {
                ...
                model.DietaryExposurePercentiles = percentages
                ...
                if (numUncertaintyRecords > 0) {
                    for (int i = 0; i < numUncertaintyRecords; i++) {
                        ...
                        for (int j = 0; j < percentages.Length; j++) {
                            model.DietaryExposurePercentiles[percentages[j]].ExposureUncertainties.Add(percentiles[j]);
                        }
                    }
                }
                result.Add(model);
