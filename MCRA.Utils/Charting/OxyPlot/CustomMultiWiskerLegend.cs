...
namespace MCRA.Utils.Charting.OxyPlot {
    public class CustomMultiWiskerLegend : LegendBase {

        private OxyRect legendBox;
        /// <summary>
        /// Initializes a new insance of the Legend class.
        /// </summary>
        public CustomMultiWiskerLegend() {
        ...
        private OxySize RenderOrMeasureLegends(IRenderContext rc, OxyRect rect, bool measureOnly = false) {
            var multiWiskerSeries = GetMultiWiskerSeries();
            ...
            foreach (var serie in multiWiskerSeries) {
                var textSize = rc.MeasureMathText(serie.Title, this.LegendFont ?? this.PlotModel.DefaultFont, actualLegendFontSize, this.LegendFontWeight);
                ...
                if (!measureOnly) {
                    ...
                    rc.DrawRectangle(new OxyRect(xBox, yBox, this.LegendSymbolLength, this.LegendSymbolWidth), serie.Fill, OxyColors.Undefined, 1, EdgeRenderingMode.Automatic);
                    ...
                    var charsPerWidth = serie.Title.Length / textSize.Width;
                    var maxChars = (int)(maxTxtWidth * charsPerWidth);
                    var nrOfChars = Math.Max(serie.Title.Length, maxChars);
                    var label = serie.Title.LimitTo(nrOfChars);
        ...
        private List<MultipleWhiskerHorizontalBoxPlotSeries> GetMultiWiskerSeries() {
            return this.PlotModel.Series.Select(c => c as MultipleWhiskerHorizontalBoxPlotSeries).ToList();
        }
    }
}
