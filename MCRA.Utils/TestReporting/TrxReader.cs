...
namespace MCRA.Utils.TestReporting {
    public sealed class TrxReader {

        /// <summary>
        /// This class reads a trx file and produces a list of TestResultSummaryRecords that describe the test results
        /// of the tests summarized in the trx file.
        /// </summary>
        /// <param name="filename"></param>
        /// <returns></returns>
        public TestResultSummary Read(string filename) {
            var fileInfo = new FileInfo(filename);
            var testResultSummaryDictionary = new Dictionary<string, TestResultSummaryRecord>();
            using (var fileStreamReader = new StreamReader(fileInfo.FullName)) {
                var xmlSerializer = new XmlSerializer(typeof(TestRunType));
                var testRunType = (TestRunType)xmlSerializer.Deserialize(fileStreamReader);
                string startDate = null;
                foreach (var testRunTypeItem in testRunType.Items) {
                    tryParseTestDefinition(testResultSummaryDictionary, testRunTypeItem);
                    if (startDate == null) {
                        startDate = tryParseTestStartDate(testRunTypeItem);
                    }
                }
                foreach (var testRunTypeItem in testRunType.Items) {
                    tryParseTestResults(testResultSummaryDictionary, testRunTypeItem);
                }
                return new TestResultSummary() {
                    TestResultSummaryRecords = testResultSummaryDictionary.Values.ToList(),
                    StartDate = startDate
                };
            }
        }

        private static void tryParseTestResults(Dictionary<string, TestResultSummaryRecord> testResultSummaryDictionary, object testRunTypeItem) {
            var resultsType = testRunTypeItem as ResultsType;
            if (resultsType != null) {
                foreach (var itob2 in resultsType.Items) {
                    ...
                    testResultSummaryDictionary[id].Outcome = unitTestResultType.outcome;
                    testResultSummaryDictionary[id].Duration = unitTestResultType.duration;
                }
            }
        }

        private static void tryParseTestDefinition(Dictionary<string, TestResultSummaryRecord> testResultSummaryDictionary, object testRunTypeItem) {
            var testDefinitionType = testRunTypeItem as TestDefinitionType;
            if (testDefinitionType != null) {
                foreach (var itob2 in testDefinitionType.Items) {
                    var testType = itob2 as UnitTestType;
                    if (testType != null) {
                        var summary = new TestResultSummaryRecord() {
                            TestId = testType.id,
                            TestName = testType.name,
                            ClassName = testType.TestMethod.className
                        };
                        testResultSummaryDictionary.Add(summary.TestId, summary);
                    }
                }
            }
        }

        private static string tryParseTestStartDate(object testRunTypeItem) {
            var testRunTypeTimes = testRunTypeItem as TestRunTypeTimes;
            if (testRunTypeTimes != null) {
                return testRunTypeTimes.start;
            }
            return null;
        }
    }

    public sealed class TestResultSummaryRecord {
    ...
    #region auto-generated trx-file object classes
