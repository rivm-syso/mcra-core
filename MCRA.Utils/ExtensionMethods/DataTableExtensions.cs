...
namespace MCRA.Utils.ExtensionMethods {
    public static class DataTableExtensions {
        ...
        /// <summary>
        /// Fast serializer for serializing a <see cref="DataTable"/> to binary form
        /// This method uses an intermediate data view to order records
        /// </summary>
        /// <param name="tbl"></param>
        /// <param name="orderBy">String to order the datatable, </param>
        /// <param name="tableSchema"></param>
        /// <returns></returns>
        public static byte[] FastSerialize(this DataTable tbl, IEnumerable<string> orderBy, out string tableSchema) {
            var tableItems = new object[tbl.Rows.Count][];

            //if ordering is requested, create an ordered view of the datatable and
            //populate the tableItems from that
            if(orderBy.Any()) {
                var view = tbl.DefaultView;
                view.Sort = string.Join(", ", orderBy);
                var rowIndex = 0;
                foreach(DataRowView rowView in view) {
                    tableItems[rowIndex++] = rowView.Row.ItemArray;
                }
            } else {
                for (var i = 0; i < tbl.Rows.Count; i++) {
                    tableItems[i] = tbl.Rows[i].ItemArray;
                }
            }
        ...
        /// <summary>
        /// Fast deserializer for deserializing a <see cref="DataTable"/> from binary form
        /// </summary>
